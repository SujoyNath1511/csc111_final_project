...
def draw_choose_players(screen: pygame.Surface) -> None:
    ...
    checkers_pygame.draw_text(screen, ' Start the Game', (5 * OFFSET, OFFSET + rect_height // 3), 26)
    ...
    for x in range(0, len(PLAYER_TYPES)):
        ...
        checkers_pygame.draw_text(screen, ' ' + PLAYER_TYPES[x],
                  (3 * OFFSET, OFFSET + (x + 1) * rect_height + rect_height // 3), 26)
...
def green_box(p1: tuple, p2: tuple, screen: pygame.Surface) -> None:
    ...
    if p1[2] != '':
        ...
        checkers_pygame.draw_text(screen, ' ' + p1[0],
                  (p1[1] * OFFSET, OFFSET + (p1[2] + 1) * rect_height + rect_height // 3), 26)
        pygame.display.update()
    if p2[2] != '':
        ...
        checkers_pygame.draw_text(screen, ' ' + p2[0],
                  (p2[1] * OFFSET, OFFSET + rect_height // 3 + (p2[2] + 1) * rect_height), 26)
        pygame.display.update()

def look_back_to_moves(moves: list, screen: pygame.Surface) -> None:
    asdfa


def info_page() -> None:
    screen = checkers_pygame.initialize_screen((600, 600), [pygame.quit()])
